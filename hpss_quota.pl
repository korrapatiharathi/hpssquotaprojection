...
my $dbh = DBI->connect(join('',
          ...
          qw{
            ...
            $user, $pass);
my $query = "select CASE WHEN bytes > quota THEN '*' ELSE '' END as OQ, name, CASE WHEN type = 'u' THEN 'User' WHEN type = 'p' THEN 'Projects' ELSE type END as type, id, format(bytes,0) as bytes, format(quota,0) as quota from quotas_hpss where ctime >= $today_unix_time_6_am AND ctime <= $today_unix_time_5_pm AND (bytes <> 0 AND quota <>0) order by type, name";
